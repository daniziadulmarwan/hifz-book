...
import ModalAddManzil from "../ModalAddManzil";

function TableManzil({ santri_id }) {
  const [openModalAddManzil, setOpenModalAddManzil] = useState(false);
  const [datas, setDatas] = useState([]);

  const getManzilBySantriId = (santri_id) => {
    axiosJwt.get(`/manzil/getAllManzilBySantriId/${santri_id}`).then((res) => {
      setDatas(res.data.data);
    });
  };

  useEffect(() => {
    getManzilBySantriId(santri_id);
  }, [santri_id]);
  ...
  return (
    <>
      ...
      <div className="flex justify-between md:items-center gap-3">
        <div></div>
        <button
          onClick={() => setOpenModalAddManzil(true)}
      ...
      <ModalAddManzil
        santri_id={santri_id}
        openModalAddManzil={openModalAddManzil}
        setOpenModalAddManzil={setOpenModalAddManzil}
      />
    </>
  );
}

export default TableManzil;
